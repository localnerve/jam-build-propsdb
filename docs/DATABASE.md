...
|---------|-------|--------------|--------------------|------------------|---------------------|
| **mariadb** | `mariadb:11.4` | 3306 | `/var/lib/mysql` | `MYSQL_ROOT_PASSWORD`, `MYSQL_DATABASE`, `MYSQL_USER`, `MYSQL_PASSWORD` | `healthcheck.sh --connect --innodb_initialized` |
...
| **postgres** | `postgres:17-alpine` | 5432 | `/var/lib/postgresql/data` | `POSTGRES_PASSWORD`, `POSTGRES_DB`, `POSTGRES_USER` | `pg_isready -U ${POSTGRES_USER} -d ${POSTGRES_DB}` |
